...
interface FormState {
  product_name: string; brand: string; category_id: string; supplier_product_code: string
  summary: string; description: string; mobile_description: string
  supply_price: string; sale_price: string; stock_qty: string
  min_order_qty: string; max_order_qty: string; is_option: boolean
  sale_start_at: string; sale_end_at: string
}
interface ScmShippingState {
  sc_type: ScType; sc_price: string; sc_minimum: string; sc_condition: 'amount' | 'qty'
  sc_qty: string; delivery_days: string; return_fee: string; exchange_fee: string; delivery_co: string
}
interface OptionDraft {
  key: string; id?: string; option_name: string; option_group?: string
  add_price: string; stock_qty: string
}
interface OptionGroupsLoad { option_mode?: string; legacy_options?: LegacyOption[] }
...
async function persistLegacyOptions(productId: string, drafts: OptionDraft[]) {
  for (const d of drafts) {
    ...
    if (!name) continue
    const body = { option_name: name, add_price: parseFloat(d.add_price) || 0, stock_qty: parseInt(d.stock_qty, 10) || 0 }
    if (d.id) { await updateOption(productId, d.id, body) }
    else { await scmPost(`/products/${encodeURIComponent(productId)}/options`, { ...body, sort_order: 0 }) }
  }
}

const EMPTY_FORM: FormState = {
  product_name: '', brand: '', category_id: '', supplier_product_code: '',
  summary: '', description: '', mobile_description: '',
  supply_price: '0', sale_price: '0', stock_qty: '99999',
  min_order_qty: '1', max_order_qty: '', is_option: false,
  sale_start_at: '', sale_end_at: '',
}
const EMPTY_SHIP: ScmShippingState = {
  sc_type: 3, sc_price: '0', sc_minimum: '', sc_condition: 'amount',
  sc_qty: '1', delivery_days: '2', return_fee: '0', exchange_fee: '0', delivery_co: '',
}

function scmShippingToPolicy(s: ScmShippingState): ShippingPolicyValue {
  return {
    sc_type: String(s.sc_type), sc_method: '0', sc_price: s.sc_price,
    sc_minimum: s.sc_condition === 'amount' ? s.sc_minimum : '',
    sc_free_qty: s.sc_condition === 'qty' ? s.sc_minimum : '',
    sc_qty: s.sc_qty, sc_condition_type: s.sc_condition,
    return_fee: s.return_fee, exchange_fee: s.exchange_fee,
    delivery_company: s.delivery_co, delivery_days: s.delivery_days,
  }
}
function policyToScmShipping(v: ShippingPolicyValue): ScmShippingState {
  const cond = v.sc_condition_type
  return {
    sc_type: Number(v.sc_type) as ScType, sc_price: v.sc_price,
    sc_minimum: cond === 'amount' ? v.sc_minimum : (v.sc_free_qty || v.sc_minimum),
    sc_condition: cond, sc_qty: v.sc_qty, delivery_days: v.delivery_days,
    return_fee: v.return_fee, exchange_fee: v.exchange_fee, delivery_co: v.delivery_company,
  }
}
...
export default function ProductFormView({ mode = 'create', productId }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const dUrls = imgs.filter(i => i?.image_type !== 'THUMBNAIL' && i?.image_url).map(i => String(i.image_url))
        ...
        setForm({
          ...
          stock_qty: String(product.stock_qty ?? 0), min_order_qty: String(product.min_order_qty ?? 1),
          max_order_qty: product.max_order_qty != null ? String(product.max_order_qty) : '',
          is_option: Boolean(product.is_option),
          sale_start_at: toDatetimeLocalInput(product.sale_start_at), sale_end_at: toDatetimeLocalInput(product.sale_end_at),
        })
        ...
        const sc_condition: ScmShippingState['sc_condition'] = rawCond === 'qty' || rawCond === 'QTY' ? 'qty' : 'amount'
        setShipping({
          sc_type: Number(product.sc_type ?? 3) as ScType,
          sc_price: String(product.sc_price ?? product.shipping_fee ?? 0),
          sc_minimum: product.sc_minimum != null ? String(product.sc_minimum) : '', sc_condition,
          sc_qty: product.sc_qty != null ? String(product.sc_qty) : '1',
          delivery_days: String(product.delivery_days ?? 2),
          return_fee: String(product.return_fee ?? 0), exchange_fee: String(product.exchange_fee ?? 0),
  ...
  const buildScmPayload = useCallback((): Record<string, unknown> => {
    ...
    return {
      ...
      supply_price: supplyN, sale_price: saleN > 0 ? saleN : 1,
      ...
      sc_type: shipping.sc_type, sc_price: parseFloat(shipping.sc_price) || 0,
  ...
  const createDraftForOptions = useCallback(async (): Promise<string | null> => {
    try {
      const data = await scmPost<{ product_id?: string }>('/scm/products', {
        ...buildScmPayload(), product_name: form.product_name.trim() || '(임시) 옵션 상품', is_option: true,
      })
      const id = data?.product_id ? String(data.product_id) : null
      if (!id) { toast('임시 저장에 실패했습니다.', 'error'); return null }
      return id
    } catch (e: unknown) {
      toast(e instanceof Error ? e.message : '임시 저장에 실패했습니다.', 'error'); return null
    }
  }, [buildScmPayload, form.product_name, toast])

  const handleIsOptionChange = useCallback(async (checked: boolean) => {
    if (!checked) { setForm(f => ({ ...f, is_option: false })); return }
    if (resolvedProductId) { setForm(f => ({ ...f, is_option: true })); return }
    ...
    try {
      ...
      if (effectiveMode === 'create' && !pid) {
        ...
        const patchBody = {
          ...
          thumbnail_url: payload.thumbnail_url, detail_images: payload.detail_images,
          supply_price: supplyN, sale_price: saleN,
          stock_qty: payload.stock_qty, min_order_qty: payload.min_order_qty,
          max_order_qty: payload.max_order_qty,
          sc_price: payload.sc_price, delivery_days: payload.delivery_days,
      ...
      if (msg.includes('APPROVED') || msg.includes('승인')) {
        toast('승인 완료 상품은 수정할 수 없습니다.', 'error')
    ...
    <>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Chip label='일반상품 (001)' color='primary' size='small'
            icon={<i className='tabler-package' style={{ fontSize: 14 }} />} />
        </Box>
        <Divider />
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'center', p: 2, borderRadius: 2, bgcolor: 'action.hover', border: '1px solid', borderColor: 'divider' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Box>
              ...
              <Typography variant='body2' fontWeight={700}
                color={approvalStatus === 'APPROVED' ? 'success.main' : approvalStatus === 'REJECTED' ? 'error.main' : 'warning.main'}>
                {approvalStatus === 'APPROVED' ? '승인완료 (수정 불가)' : approvalStatus === 'REJECTED' ? '반려 (수정 후 재심사)' : '승인대기 (관리자 검토 중)'}
              </Typography>
            </Box>
          </Box>
        </Box>
            ...
            {mode === 'edit' && (
              <Grid size={{ xs: 12, sm: 6 }}>
                <CustomTextField fullWidth size='small' label='상품코드 (자사)' disabled
                  value={productCode || '—'} helperText='8자리 자사코드. 시스템 자동 생성·수정 불가' />
              </Grid>
            )}
            <Grid size={{ xs: 12, sm: 6 }}>
              <CustomTextField fullWidth size='small' label='공급사 상품코드'
                value={form.supplier_product_code} placeholder='공급사/KMC 상품코드 (선택)'
                helperText={mode === 'create' ? '자사 상품코드는 저장 후 자동 생성됩니다' : undefined}
                onChange={e => setForm(f => ({ ...f, supplier_product_code: e.target.value }))} />
            </Grid>
            <Grid size={12}>
              <CustomTextField required fullWidth size='small' label='상품명' value={form.product_name}
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 4 }}>
              <CustomTextField fullWidth size='small' label='공급가 (원)' type='number'
                value={form.supply_price} onChange={e => setForm(f => ({ ...f, supply_price: e.target.value }))}
                slotProps={{ htmlInput: { min: 0, step: 'any' } }} helperText='공급사 공급 가격' />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <CustomTextField select fullWidth size='small' label='세금유형' value={taxType}
                onChange={e => setTaxType(e.target.value)}
                helperText={taxTypeOptions.length ? 'system_codes tax_type' : '과세/면세 선택'}>
                {taxTypeOptions.length > 0
                  ? taxTypeOptions.map(o => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)
                  : <><MenuItem value='0'>과세 (10%)</MenuItem><MenuItem value='1'>면세</MenuItem></>}
              </CustomTextField>
            </Grid>
            <Grid size={12}>
              <Typography variant='body2' color={profit >= 0 ? 'primary' : 'error'}>
                예상 이익금: {profit.toLocaleString('ko-KR')}원
                <Typography component='span' variant='caption' color='text.secondary' sx={{ ml: 1 }}>(판매가 - 공급가)</Typography>
              </Typography>
            </Grid>
          </Grid>
        </Box>
        <Divider />
        <ShippingPolicySection mode='scm' value={scmShippingToPolicy(shipping)}
          onChange={v => {
            setShipping(prev => {
              const next = policyToScmShipping(v)
              if (v.sc_price !== String(prev.sc_price)) {
                const price = parseFloat(v.sc_price) || 0
                next.return_fee = String(price * 2); next.exchange_fee = String(price * 2)
              }
              return next
            })
          }} />
        ...
        <StockOrderSection
          ...
          onChange={s => setForm(f => ({ ...f, ...s }))} sectionLabel='⑤ 재고·주문' />
        <Divider />
        <SaleScheduleSection sectionLabel='⑥ 예약 조건' hidePeriodToggle
          value={{ sale_start_date: form.sale_start_at, sale_end_date: form.sale_end_at, use_schedule: true }}
        ...
        <Box>
          ...
          <FormControlLabel
            control={<Switch checked={form.is_option} disabled={optionDraftSaving || isApprovedLocked}
          ...
          {form.is_option && resolvedProductId && (
            <Box sx={{ mt: 2, mb: 1, p: 2, border: '1px dashed', borderColor: 'primary.main', borderRadius: 2 }}>
              <Typography variant='body2' color='primary.main' sx={{ mb: 1, fontWeight: 600 }}>2단 조합 옵션</Typography>
          ...
          {form.is_option && !resolvedProductId && optionDraftSaving && (
            <Box sx={{ mt: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Typography variant='caption' color='text.secondary'>옵션 설정을 위해 상품을 임시 저장하는 중…</Typography>
            </Box>
          )}
        </Box>
        <Divider />
        <Box>
          <Typography variant='subtitle2' sx={{ mb: 2, color: 'text.secondary' }}>
            ⑧ 대표 이미지<Typography component='span' variant='caption' sx={{ ml: 1, color: 'error.main' }}>* 필수</Typography>
          </Typography>
          <ImageUploader bucket='products' folder='thumbnails' value={thumbnailUrls} onChange={setThumbnailUrls} maxCount={1} />
          {thumbnailUrls.length === 0 && (
            <Typography variant='caption' color='error' sx={{ mt: 0.5, display: 'block' }}>대표 이미지를 업로드해 주세요.</Typography>
          )}
        </Box>
        <Divider />
        <Box>
          <Typography variant='subtitle2' sx={{ mb: 2, color: 'text.secondary' }}>
            ⑨ 상세 이미지<Typography component='span' variant='caption' color='text.secondary' sx={{ ml: 1 }}>(최대 10장)</Typography>
          </Typography>
          <ImageUploader bucket='products' folder='detail' value={detailUrls} onChange={setDetailUrls} maxCount={10} />
        </Box>
        <Divider />
        <Box>
          ...
          <Box>
            ...
            <RichEditor value={form.description} onChange={v => setForm(f => ({ ...f, description: v }))}
              placeholder='상품 상세설명을 입력하세요.' minHeight={240} uploadBucket='products' uploadFolder='editor' />
          </Box>
          <Box sx={{ mt: 2 }}>
            <Button size='small' variant='text' onClick={() => setMobileDescriptionOpen(o => !o)}
            ...
            <Collapse in={mobileDescriptionOpen}>
              ...
              <RichEditor value={form.mobile_description} onChange={v => setForm(f => ({ ...f, mobile_description: v }))}
                placeholder='모바일용 상세설명' minHeight={200} uploadBucket='products' uploadFolder='editor' />
            </Collapse>
          </Box>
        </Box>
