...
interface DashboardData {
  pending_products: number  // 승인대기 상품 수
  new_orders: number        // 신규 주문 수
  this_month_sales: number  // 이번달 매출
  unsettled_amount: number  // 미정산 금액
  recent_orders?: Array<{ order_no: string; status: string; total_amount: number; created_at: string }>
}

const STAT_CARDS = [
  { key: 'pending_products', label: '승인대기 상품', icon: 'tabler-clock', color: 'warning.main', href: '/products', unit: '건' },
  { key: 'new_orders',       label: '신규 주문',     icon: 'tabler-shopping-bag', color: 'primary.main', href: '/orders', unit: '건' },
...
export default function DashboardView() {
  ...
  useEffect(() => {
    scmGet<DashboardData>('/scm/dashboard')
      .then(d => setData(d))
      .catch(e => {
        // 대시보드 API 없을 경우 mock 데이터로 fallback
        setData({ pending_products: 0, new_orders: 0, this_month_sales: 0, unsettled_amount: 0 })
        if (String(e.message).includes('요청 실패')) setError(String(e.message))
      })
  ...
  return (
    <Box className='flex flex-col gap-6'>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {data?.pending_products != null && data.pending_products > 0 && (
          <Chip label={`승인대기 ${data.pending_products}건`} color='warning' size='small'
            onClick={() => router.push('/products')} sx={{ cursor: 'pointer', fontWeight: 700 }} />
        )}
      </Box>
        ...
        <Grid container spacing={3}>
          {STAT_CARDS.map(card => (
            <Grid key={card.key} item xs={12} sm={6} md={3}>
              ...
              >
                <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 1, pb: '16px !important' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Box sx={{
                      width: 44, height: 44, borderRadius: 2, bgcolor: card.color,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    }}>
                      <i className={card.icon} style={{ fontSize: 22, color: '#fff' }} />
                    </Box>
                    <Box>
                      <Typography variant='caption' color='text.secondary'>{card.label}</Typography>
                      <Typography variant='h5' fontWeight={700}>
                        {card.isMoney
                          ? fmtMoney((data as Record<string, number>)[card.key])
                          : `${((data as Record<string, number>)[card.key] ?? 0).toLocaleString('ko-KR')}${card.unit ?? ''}`
                        }
                      </Typography>
                    </Box>
                  </Box>
        ...
        <CardContent>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5 }}>
            {QUICK_LINKS.map(link => (
              <Button key={link.href} variant='outlined' color={link.color} size='small'
          ...
          <CardContent>
            ...
            {data.recent_orders.map(o => (
              <Box key={o.order_no} sx={{
                ...
                <Typography variant='body2' color='text.secondary'>{o.created_at?.slice(0, 10)}</Typography>
